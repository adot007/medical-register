...
if (
   ...
   /*  if(!isset($_POST['relation'])){
    ...
    if (!$result) {
        ...
        $appointmentSqlQuery = "INSERT INTO appointments (patient_id)
                                 VALUES ('$current_patient_id');"; 
                                
        if (mysqli_query($conn, $appointmentSqlQuery)){
            echo "<alert>Appointment has been set for this patient... </alert>";

        } else {
            echo "Error forwarding patient's information to doctor: " . mysqli_error($conn);
        }
